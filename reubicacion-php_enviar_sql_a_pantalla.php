...
procesos.pro_descripcion AS proceso_del_equipo, reu_responsable AS responsable, reu_usuario AS usuario, reu_bloque AS bloque,
...
reu_ot_sigma AS ot_sigma, reu_observacion AS observaciones, reu_f_ult_actualizacion AS Fecha_ultima_actualizacion FROM (SELECT @numero:=0) r,reubicaciones
...
WHERE reu_activo LIKE '%$activo_equipo%' AND reu_activo_equipo_retirar LIKE '%$activo_equipo_retirar%' AND  reu_responsable LIKE '%$responsable%' AND  reu_nuevo_responsable LIKE '%$nuevo_responsable%' AND  reu_estado_equipo LIKE '%$id_est%'";
